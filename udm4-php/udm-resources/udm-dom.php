...
foreach($umary as $data)
{
	...
	{
		$umjs.=preg_match("~^[+\-]?[0-9]+$~",$subdata)?"$subdata,":"'$subdata',";
		//if this value is an image then remember it
		if(preg_match("~(gif|png|mng|jpg|jpeg|jpe|bmp)~i",$subdata)) { $umimgs[$m++]=$subdata; }
	}
}
$umjs.="];";
...
foreach($um["itemClasses"] as $key=>$data)
{
	...
	foreach($data as $subkey=>$subdata)
	{
		$umjs.="'$subdata',";
		//if this value is an image then remember it
		if(preg_match("~(gif|png|mng|jpg|jpeg|jpe|bmp)~",$subdata)) { $umimgs[$m++]=$subdata; }
	}
	$umjs.="];";
}
...
if(isset($um['trigger'])){$umjs.='um.trigger=\''.$um['trigger'].'\';';}
...
if($umkb)
{
	...
	foreach($um['keys'] as $data)
	{
		...
		$umjs.=preg_match("~^[0-9]+$~",$data)?"$data,":"'$data',";
	}
	$umjs.="];";
}
...
$umjs.='um.kb=('.($umkb?'true':'false').'&&!(um.mie||um.o7||(um.k&&!um.k4)||(um.s&&!um.s2)));um.skb=(um.kb||('.($umkb?'true':'false').'&&((um.o7&&!um.o71)||(um.k&&!um.k4))));um.sp=('.($umsp?'true':'false').'&&um.wie);if(um.wie50&&um.r){um.e[12]=\'no\';}um.rp='.($um['orientation'][3]=='relative'?'true':'false').';um.p='.($ump?'true':'false').';um.hz=((um.wie50&&'.($um['behaviors'][3]=='default'?'true':'false').')||(um.wie&&'.($um['behaviors'][3]=='hide'?'true':'false').'));um.a='.($um['orientation'][1]=='right'?'true':'false').';um.h='.($um['orientation'][0]=='horizontal'?'true':'false').';um.rg=(um.h&&'.($um['list'][0]=='rigid'?'true':'false').'&&'.($umdir!='right'?'true':'false').');um.ep=0;if('.($um['orientation'][0]=='expanding'?'true':'false').'){um.ep=1;um.e[0]=\'vertical\';}um.fe=false;if(um.e[3]==\'allfixed\'){um.e[3]=\'fixed\';if(um.wie5||um.wie6){um.fe=true;}}um.f=(um.e[3]==\'fixed\'&&!(um.wie5||um.wie6||um.og));um.nc='.(($um['items'][0]==0&&$um['items'][2]=='collapse')?'true':'false').';um.mc='.($um['menuItems'][0]==0&&$um['menuItems'][2]=='collapse'?'true':'false').';um.nm=((um.og&&um.rp)||(um.omie&&um.h)||((um.dg||um.wie50)&&'.($umdir=='right'?'true':'false').'));um.nr=(um.nm||um.mie);um.ns=(um.dg||um.o71||(um.wie50&&um.rp)||(um.o7&&um.f)||um.mie);um.cns=(typeof um.m.createElementNS!=um.un);um.ss=(um.cns&&typeof um.m.styleSheets!=um.un&&!(um.s||um.k));um.ni='.(preg_match("~(gif|png|mng|jpg|jpeg|jpe|bmp)~i",$um['items'][28])?'true':'false').';um.mi='.(preg_match("~(gif|png|mng|jpg|jpeg|jpe|bmp)~i",$um['menuItems'][28])?'true':'false').';um.rn=0;um.rv=[];um.addReceiver=function(umFC,umEC){um.rv[um.rn++]=[umFC,umEC];};um.createElement=function(umE,umA){um.el=(um.cns)?um.m.createElementNS(\'http://www.w3.org/1999/xhtml\',umE):um.m.createElement(umE);if(typeof umA!=um.un){for(var i in umA){switch(i){case \'text\' :um.el.appendChild(um.m.createTextNode(umA[i]));break;case \'class\' : um.el.className=umA[i];break;default : um.el.setAttribute(i,\'\');um.el[i]=umA[i];break;}}}return um.el;};';
...
$umjd[$j++]="um.ap=function(c,v){var r=um.rv.length;if(r>0){for(var i=0;i<r;i++){if(um.rv[i][1]==''){um.rv[i][0](v,c);}else if(c==um.rv[i][1]){um.rv[i][0](v);}}}};if(um.wie){um.eva=[];um.ex=['onmouseover','onmouseout','onmousedown','onmouseup','onclick','onmousewheel','onfilterchange','onkeydown','onfocus','onactivate','onscroll','over','out'];um.gg=um.ex.length;window.attachEvent('onunload',function(){um.lil=umTree.getElementsByTagName('li');um.lin=um.lil.length;i=0;do{um.gc(um.lil[i]).detachEvent((um.wie55)?'onactivate':'onfocus',um.eva[i]);i++;}while(i<um.lin);um.da=document.all.length;i=0;do{um.t=document.all[i];j=0;do{um.t[um.ex[j]]=null;j++;}while(j<um.gg);i++;}while(i<um.da);});}";
$umjd[$j++]="if(!um.k&&typeof window.addEventListener!=um.un){window.addEventListener('load',umIni,0);}else if(um.o7){um.m.addEventListener('load',umIni,0);}else if(um.wie){window.attachEvent('onload',umIni);}else{if(typeof window.onload=='function'){um.on=onload;window.onload=function(){um.on();umIni();};}else{window.onload=umIni;}}";
$umjd[$j++]="function umIni(g){if(typeof g==um.un){g=1;}if(typeof um.ini!=um.un||(um.k&&typeof window.sidebar==um.un)){return;}if(um.drt){clearTimeout(um.drt);}um.ini=1;um.ha=0;umTree=(um.b)?um.gd('udm'):null;if(umTree&&um.d){if(g){um.ap('000',umTree);}";
$umjd[$j++]="for(i in um.menuCode){var l=um.gd(i);if(l){if(um.mie){um.menuCode[i]=um.menuCode[i].replace(/<\/(li|ul)>/ig,'</\$1>\\n');}l.innerHTML+=um.menuCode[i];if(um.mie){um.dm=um.gm(l);um.xn(um.dm);um.xh(um.dm);}}}";
...
if($umkb) 
{
	$umjd[$j++]="if(um.skb){um.kbm=new umKM;if(g){um.ap('001',um.kbm);}}";
}
$umjd[$j++]="um.sp=(um.sp&&typeof udmSpeechModule=='function');";
if($umsp) 
{
	$umjd[$j++]="if(um.sp){um.sapi=new udmSpeechModule;if(g){um.ap('002',um.sapi);}}";
}
$umjd[$j++]="um.n=new umNav(umTree,g);if(g){um.ap('009',um.n);}if(um.fe){um.tr.style.left=(um.getScrollAmount(1))+'px';um.tr.style.top=(um.getScrollAmount())+'px';window.attachEvent('onscroll',function(){um.tr.style.left=(um.getScrollAmount(1))+'px';um.tr.style.top=(um.getScrollAmount())+'px';});}if(um.s){umTree.style.KhtmlOpacity='1';}um.s1=(typeof umTree.style.KhtmlOpacity!=um.un);um.ready=1;if(g){um.ap('010',um.tr);}}};";
$umjd[$j++]="function umNav(umTree,g){um.n=this;um.tr=umTree;if(um.wie){um.tr.style.color='black';}um.jv='javascript:void(0)';";
if($umrw) 
{
	$umjd[$j++]="if(um.rg){um.rw=0;}";
}
$umjd[$j++]="var l=umTree.getElementsByTagName('li');if(l.length==0){return;}var i=0;do{";
if(count($um['menuClasses'])>0) 
{
	$umjd[$j++]="if(um.wl>0){var b=um.es(l[i].className);if(b==''&&!um.ne(l[i])){var a=um.gp(l[i].parentNode);b=um.es(a.className);if(b!=''&&!um.ne(a)){l[i].className=b;}}}";
}
$umjd[$j++]="this.it(l[i]);if(g){um.ap('008',l[i]);}i++;}while(i<l.length);";
if(count($um['itemClasses'])>0) 
{
	$umjd[$j++]="if(um.vl>0){um.mo=um.gu(um.tr);um.en=um.mo.length;if(um.en>0){i=0;do{b=um.es(um.mo[i].className);if(b==''){a=um.mo[i].parentNode.parentNode;b=um.es(a.className);if(b!=''&&b!='udm'){um.mo[i].className=b;}}i++;}while(i<um.en);}}";
}
...
$umjd[$j++]="um.cc=null,um.cr=0,um.oc=null,um.or=0;if(!um.ie){um.tr.contains=function(n){return (n==null)?false:(n==this)?true:this.contains(n.parentNode);};}um.lw=um.getWindowDimensions();um.lh=um.gc(um.tr).offsetHeight;if(um.og&&um.hstrip[0]!='none'){um.tr.style.height=(um.hstrip[1]=='yes')?(um.lh+um.e[17])+'px':um.lh+'px';}var p=um.m.getElementById('udm-purecss');if(p){p.disabled=1;}um.vs=setInterval('um.n.ws()',55);};";
$umjd[$j++]="umNav.prototype.it=function(l){if(um.wie){var f=(um.wie55)?'onactivate':'onfocus';um.gc(l).attachEvent(f,um.eva[um.eva.length]=function(){";
if($umkb) 
{
	$umjd[$j++]="if(um.kb&&!um.lf){um.bub=0;l.over(1,um.gc(l));}";
}
$umjd[$j++]="});}var a=um.es(l.className);var h=(a.indexOf('onclick')!=-1)?'onclick':'onmouseover';var s=um.ne(l);var umM=(typeof um.gu(l)[0]!=um.un)?um.gu(l)[0]:null;if(typeof um.fl==um.un){um.fl=um.gc(l);}";
if($umaz) 
{
...
$umjd[$j++]="if(um.mie){var v=l.getElementsByTagName('span')[0];if(typeof v!=um.un){v.onclick=function(){this.parentNode.click();};}}";
if($umrw) 
{
	$umjd[$j++]="if(um.rg&&um.ne(l)){um.n.dw(l);}";
}
$umjd[$j++]="if(um.mie){t=um.gc(l);if(t.className&&/nohref/.test(t.className)){um.gc(l).href=um.jv;}}";
if($umkb) 
{
	$umjd[$j++]="if(um.skb){um.kbm.bth(l);}";
}
$umjd[$j++]="l.onmousedown=function(e){um.lf=1;um.ap('030',um.gc(this));(!e)?event.cancelBubble=1:e.stopPropagation();};l.onmouseup=function(e){um.ap('035',um.gc(this));(!e)?event.cancelBubble=1:e.stopPropagation();};if(h!='onclick'){l.onclick=function(e){if(!um.bub){um.qc(um.gc(this).href);}um.bub=1;};}else if(!um.mie){l.onmouseover=function(){um.n.lr(um.gc(l),1);um.bub=0;};}if(!(um.mie&&h=='onclick')){l[h]=function(e){var v=(um.ie)?window.event.srcElement:e.target;if(v.nodeName=='#text'&&e.type=='click'){v=v.parentNode;}if(!um.gp(v)){return false;}var b=um.es(um.gp(v).className);var c=(um.lf&&!um.nm&&b.indexOf('onclick')!=-1);if(c){um.rt=um.e[10];um.e[10]=1;}if(b.indexOf('onclick')==-1){um.bub=0;}else if(!um.lf){if(!um.bub){um.qc(v.href);}um.bub=1;}this.over(0,v);if(c){um.e[10]=um.rt;um.lf=0;if(v.nodeName!='#text'&&um.gu(um.gp(v)).length>0){if(typeof v.blur!=um.un){v.blur();}if(um.gu(um.gp(v))[0].style.display=='block'){um.n.cd(this.parentNode);(!e)?event.cancelBubble=1:e.stopPropagation();return false;}(!e)?event.cancelBubble=1:e.stopPropagation();b=um.es(um.gp(v).className);return (b.indexOf('(true)')!=-1);}else{um.qc(v.href);um.bub=1;}}if(!e){e=window.event;}return (e.type=='click'||um.o7);};l.onmouseout=function(e){this.out(e);};}l.over=function(f,t){if(um.bub||(!f&&um.ha&&um.kdf)){return false;}var c=um.n.cck();if(!c||um.mf){um.mf=0;if(!um.ec){if(um.gm(this)){this.removeChild(um.gm(this));}}return false;}";
if($umkb) 
{
	$umjd[$j++]="if(f){if(!um.wsr&&!um.ie){um.kbm.cws(um.tr);um.wsr=1;}";
if($umsp) 
{
	$umjd[$j++]="if(um.sp){um.sapi.speechBuffer(um.gc(l));event.cancelBubble=1;}";
}
$umjd[$j++]="um.ha=1;if(um.ie&&event.altKey){um.n.ck(um.gp(t).parentNode);}um.ap('040',t);}";
}
$umjd[$j++]="if(!f){var n=um.vn(t.nodeName).toLowerCase();if(/(li|ul)/.test(n)){return false;}";
if($umkb) 
{
	$umjd[$j++]="if(um.skb){if(!um.lf){um.e[10]=um.mt[0];um.e[11]=um.mt[1];}um.nf=um.gc(this);if(um.ha){um.n.ck(l.parentNode);um.n.cd(um.gp(t).parentNode);um.nf.focus();um.nf.blur();um.ha=0;}}";
}
$umjd[$j++]="um.ap('020',t);}clearInterval(um.cc);um.cr=0;um.n.lr(um.gc(l),1);um.n.pr(umM,l,f,t);return l;};l.out=function(e){if(um.o7&&um.ha&&um.kdf){return;}if(um.lf){um.gc(this).blur();}um.lf=0;if(!e){e=window.event;e.relatedTarget=e.toElement;}if(!l.contains(e.relatedTarget)){if(!um.tr.contains(e.relatedTarget)){clearInterval(um.cc);um.cr=0;}um.n.cp(umM,l);um.ap('025',um.gc(this));}};if(!um.ie){l.contains=function(n){return (n==null)?false:(n==this)?true:this.contains(n.parentNode);};}};";
...
$umjd[$j++]="umNav.prototype.lr=function(l,v){if(l&&typeof l.style!=um.un){um.cl=um.es(l.className);um.ii=um.ne(um.gp(l));if(v){l.style.zIndex=um.e[6]+=2;(um.cl=='')?l.className='udmR':l.className+=(l.className.indexOf('udmR')==-1)?' udmR':'';}else{if(um.cl.indexOf('udmR')!=-1){l.className=um.cl.replace(/([ ]?udmR)/g,'');}}";
if($umaz) 
{
	$umjd[$j++]="um.n.wv(l,um.ii);";
}
$umjd[$j++]="}};";
$umjd[$j++]="umNav.prototype.pr=function(m,l,f,r){";
if($umkb) 
{
	$umjd[$j++]="if(um.skb&&f){um.kbm.cu(m,l,r);}";
}
$umjd[$j++]="if(!um.nm&&m&&m.style.visibility!='visible'){if(um.wie&&!um.wie7){if(um.e[61]>0){um.gc(m).style.marginTop=um.e[61]+'px';}else if(um.e[63]=='collapse'){m.firstChild.style.marginTop=0+'px';}}";
if($umkb) 
{
	$umjd[$j++]="if(um.skb&&f){um.n.ou(m);}";
}
$umjd[$j++]="if(!(um.skb&&f)){um.n.tu(m,null);}}if(m==null){um.n.tu(null,l);}};";
$umjd[$j++]="umNav.prototype.tu=function(m,l){if(um.cr){clearInterval(um.oc);um.oj=m;um.ij=l;um.or=1;um.oc=setInterval('um.n.tu(um.oj,um.ij)',um.e[10]);}else if(um.or){clearInterval(um.oc);um.or=0;this.ou(m,l);}else{um.ap('061',m);um.oj=m;um.ij=l;um.or=1;um.oc=setInterval('um.n.tu(um.oj,um.ij)',um.e[10]);}};";
$umjd[$j++]="umNav.prototype.ou=function(m,l){if(m==null){this.cd(l.parentNode);return false;}this.cd(um.gp(m).parentNode);if(typeof m.m==um.un){m.m=um.gu(m);m.l=m.m.length;if(m.l>0){for(var i=0;i<m.l;i++){um.xh(m.m[i]);um.xn(m.m[i]);}}}if(um.ep){m.style.position='static';if(um.wie8){m.style.styleFloat = 'left';}}";
if($umhz) 
{
